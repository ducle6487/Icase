	...
	public String thanhtoan(User user,Model model) {
		...
		for(int i=0;i<GioHang.cart.size();i++) {
			...
			jdbctemplate.update("UPDATE product SET amount="+amount+",sold="+GioHang.cart.get(i).getSl()+"where idproduct="+GioHang.cart.get(i).getIdproduct()+"");
				jdbctemplate.update("insert into infoorder(IdOrder,IdProduct,Amount,ToTal,Color,Phone) values("+idorder.get(0).getIdOrder()+","+GioHang.cart.get(i).getIdproduct()+","+GioHang.cart.get(i).getSl()+","+totalPrice+",N'"+GioHang.cart.get(i).getColor()+"',"+device.get(0).getIdphone()+")");
	...
	public String addtoCart(@PathVariable int getIdproduct) {
		...
		{	
			for(int j=0;j<count;j++) 
			{
					for(int i=0;i<product.size();i++) 
					{						
						if((GioHang.cart.get(j).getIdproduct() == product.get(i).getIdproduct()) &&(GioHang.cart.get(i).getSl()<=GioHang.cart.get(i).getAmount())) 
						{					
							GioHang.cart.get(j).setSl(GioHang.cart.get(j).getSl()+1);
							GioHang.cart.forEach(System.out::println);
							flagnotadd = true;
