...
//		int row = jdbctemplate.update("insert into orders(DateOrder,Total,Name,Phone,Address,Message) values ("+date+","+total+",N'"+user.getName()+"',"+"0"+user.getNumber()+",N'"+ user.getAddress()+"',N'"+user.getMessage()+"')");
//		System.out.println(row);
	...
	public String product(@PathVariable int getIdphone,Model model) {
		List<Device> devices = jdbctemplate.query("select idproduct from device where idphone = ?", BeanPropertyRowMapper.newInstance(Device.class) ,getIdphone);
		List<Case> products = jdbctemplate.query("Select * from product where idproduct=?", BeanPropertyRowMapper.newInstance(Case.class),devices.get(0).getIdproduct());
	...
	public String search(Case product,Model model) {
		...
		List<Case> products = jdbctemplate.query("Select * from product where name Like N'%"+product.getName() + "%'", BeanPropertyRowMapper.newInstance(Case.class));
